...
bool platform_init_x11(Platform *platform) {
	...
	struct platform_internal *internal = platform->internal;
...
void x11_set_logical_dimensions_callback(struct platform *platform, PFN_platform_dimensions callback_fn) {
	struct platform_internal *internal = platform->internal;
	X11Platform *x11 = &internal->x11;

	x11->callback.logical_size = callback_fn;
}
void x11_set_physical_dimensions_callback(struct platform *platform, PFN_platform_dimensions callback_fn) {
	struct platform_internal *internal = platform->internal;
	X11Platform *x11 = &internal->x11;

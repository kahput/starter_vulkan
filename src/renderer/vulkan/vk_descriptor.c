...
// 		LOG_WARN("Vulkan: indices [store_index = %d, shader_index = %d] out of bounds", store_index, shader_index);
// 		return false;
// 	}
//
...
// bool vulkan_renderer_update_resource_set_buffer(VulkanContext *context, uint32_t set_index, String name, uint32_t buffer_index) {
...
//
// 	if (target == NULL) {
// 		LOG_WARN("Vulkan: Uniform buffer '%.*s' not found in shader %d", FS(name), resource_set->shader_index);
// 		return false;
// 	}
//
// 	for (uint32_t frame = 0; frame < MAX_FRAMES_IN_FLIGHT; ++frame) {
...
// }
//
// bool vulkan_renderer_update_resource_set_texture_sampler(VulkanContext *context, uint32_t set_index, String name, uint32_t texture_index, uint32_t sampler_index) {
...
//
// 	if (target == NULL) {
// 		LOG_WARN("Vulkan: Uniform texture sampler '%.*s' not found in shader %d", FS(name), resource_set->shader_index);
// 	}
//
// 	for (uint32_t frame = 0; frame < MAX_FRAMES_IN_FLIGHT; ++frame) {
...
// bool vulkan_renderer_bind_shader(VulkanContext *context, uint32_t shader_index) {
// 	VulkanResourceSet *set = &context->set_pool[retrieve_index];
// 	if (set->sets[0] == NULL) {

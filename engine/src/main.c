...
UUID create_plane_mesh(Arena *arena, uint32_t subdivide_width, uint32_t subdivide_depth,
	Orientation orientation, MeshSource **out_mesh);
...
int main(void) {
	...
	asset_library_startup(arena_push(&state.permanent, MiB(128), 1, true), MiB(128));
	...
	vulkan_renderer_resource_global_create(state.context, RENDERER_GLOBAL_RESOURCE_SHADOW,
		(ResourceBinding[]){
		  { .binding = 0, .type = SHADER_BINDING_UNIFORM_BUFFER, .size = sizeof(mat4), .count = 1 },
		},
		1);
	vulkan_renderer_resource_global_create(state.context, RENDERER_GLOBAL_RESOURCE_MAIN,
		(ResourceBinding[]){
		  { .binding = 0,
		...
		3);
	vulkan_renderer_resource_global_set_texture_sampler(state.context,
		RENDERER_GLOBAL_RESOURCE_MAIN, 1, RENDERER_DEFAULT_TARGET_SHADOW_DEPTH_MAP,
		RENDERER_DEFAULT_SAMPLER_LINEAR);

	vulkan_renderer_resource_global_create(state.context, RENDERER_GLOBAL_RESOURCE_POSTFX,
		(ResourceBinding[]){
		...
		2);
	vulkan_renderer_resource_global_set_texture_sampler(state.context,
		RENDERER_GLOBAL_RESOURCE_POSTFX, 0, state.final_texture, RENDERER_DEFAULT_SAMPLER_LINEAR);
	...
	RenderPassDesc main_pass = { .name = S("Main"),
		.color_attachments = { { .texture = RENDERER_DEFAULT_TARGET_MAIN_COLOR_MAP,
		  .clear = { .color = GLM_VEC4_BLACK_INIT },
		  .load = CLEAR,
		  .store = STORE } },
		.color_attachment_count = 1,
		.depth_attachment = { .texture = RENDERER_DEFAULT_TARGET_MAIN_DEPTH_MAP,
		  .clear = { .depth = 1.0f },
		  .load = CLEAR,
		  .store = DONT_CARE },
		.use_depth = true,
	...
	ArenaTemp scratch = arena_scratch(NULL);
	{
		...
		vulkan_renderer_shader_create(&state.permanent, state.context, default_shader_index,
			RENDERER_GLOBAL_RESOURCE_MAIN, &shader_config, &reflection);
		vulkan_renderer_shader_variant_create(state.context, default_shader_index,
...

	{
		...
		ShaderReflection reflection;
		vulkan_renderer_shader_create(&state.permanent, state.context, sprite_shader_index,
			RENDERER_GLOBAL_RESOURCE_MAIN, &sprite_config, &reflection);
		vulkan_renderer_shader_variant_create(state.context, sprite_shader_index,
			RENDERER_DEFAULT_SHADER_VARIANT_STANDARD, RENDERER_DEFAULT_PASS_MAIN, desc);
		desc.polygon_mode = POLYGON_MODE_LINE;
		vulkan_renderer_shader_variant_create(state.context, sprite_shader_index,
			RENDERER_DEFAULT_SHADER_VARIANT_WIREFRAME, RENDERER_DEFAULT_PASS_MAIN, desc);
	...
	vulkan_renderer_texture_create(state.context, sprite_texture_0, sprite_src->width,
		sprite_src->height, TEXTURE_FORMAT_RGBA8_SRGB, TEXTURE_USAGE_SAMPLED, sprite_src->pixels);

	UUID sprite_id_1 = asset_library_request_image(S("tile_0086.png"), &sprite_src);
	uint32_t sprite_texture_1 = state.next_image_index++;
	vulkan_renderer_texture_create(state.context, sprite_texture_1, sprite_src->width,
		sprite_src->height, TEXTURE_FORMAT_RGBA8_SRGB, TEXTURE_USAGE_SAMPLED, sprite_src->pixels);
	...
	vulkan_renderer_resource_group_write(
		state.context, sprite_mat_0, 0, 0, sizeof(vec4), &sprite_color, true);
	vulkan_renderer_resource_group_set_texture_sampler(
		state.context, sprite_mat_0, 0, sprite_texture_0, RENDERER_DEFAULT_SAMPLER_NEAREST);
	...
	vulkan_renderer_resource_group_write(
		state.context, sprite_mat_1, 0, 0, sizeof(vec4), &sprite_color, true);
	vulkan_renderer_resource_group_set_texture_sampler(
		state.context, sprite_mat_1, 0, sprite_texture_1, RENDERER_DEFAULT_SAMPLER_NEAREST);
	...
	if (model_src) {
		LOG_INFO("Model loaded: %d meshes, %d materials, %d images", model_src->mesh_count,
			model_src->material_count, model_src->image_count);
		...
		vulkan_renderer_texture_create(state.context, room_texture, model_src->images->width,
			model_src->images->height, TEXTURE_FORMAT_RGBA8_SRGB, TEXTURE_USAGE_SAMPLED,
			model_src->images->pixels);

		room_material = state.next_group_index++;
		vulkan_renderer_resource_group_create(
			state.context, room_material, default_shader_index, 256);
		vulkan_renderer_resource_group_write(state.context, room_material, 0, 0,
			sizeof(MaterialParameters), &default_material_parameters, true);
		vulkan_renderer_resource_group_set_texture_sampler(
			state.context, room_material, 0, room_texture, RENDERER_DEFAULT_SAMPLER_NEAREST);
	...
	UUID shadow_shader_id =
		asset_library_request_shader(S("shadow_caster.glsl"), &shadow_shader_src);
	uint32_t shadow_shader_index = state.next_shader_index++;
	{
		...
		desc.override_attributes = (ShaderAttribute[]){
			(ShaderAttribute){ .name = S("in_position"),
			  .binding = 0,
			  .format = { .type = SHADER_ATTRIBUTE_TYPE_FLOAT32, .count = 3 } },
			(ShaderAttribute){ .name = S("in_normal"),
			  .binding = 0,
			  .format = { .type = SHADER_ATTRIBUTE_TYPE_FLOAT32, .count = 3 } },
			(ShaderAttribute){ .name = S("in_uv"),
			  .binding = 0,
			  .format = { .type = SHADER_ATTRIBUTE_TYPE_FLOAT32, .count = 3 } },
			(ShaderAttribute){ .name = S("in_tangent"),
			  .binding = 0,
			  .format = { .type = SHADER_ATTRIBUTE_TYPE_FLOAT32, .count = 3 } },
		};
		...
		ShaderReflection reflection;
		vulkan_renderer_shader_create(&state.permanent, state.context, shadow_shader_index,
			RENDERER_GLOBAL_RESOURCE_SHADOW, &shadow_config, &reflection);
		vulkan_renderer_shader_variant_create(state.context, shadow_shader_index,
			RENDERER_DEFAULT_SHADER_VARIANT_STANDARD, RENDERER_DEFAULT_PASS_SHADOW, desc);
	}
	...
	uint32_t postfx_shader_index = state.next_shader_index++;
	{
		...
		vulkan_renderer_shader_variant_create(state.context, postfx_shader_index,
			RENDERER_DEFAULT_SHADER_VARIANT_STANDARD, RENDERER_DEFAULT_PASS_POSTFX, desc);
	}
	float quadVertices[] = { -1.0f, 1.0f, 0.0f, 1.0f, -1.0f, -1.0f, 0.0f, 0.0f, 1.0f, -1.0f, 1.0f,
		0.0f,
	...
	for (uint32_t light_material = 0; light_material < 16; ++light_material) {
		...
		vulkan_renderer_resource_group_create(
			state.context, light_materials[light_material], light_shader_index, 256);
	}
	...
	while (platform_should_close(&state.display) == false) {
		...
		if (vulkan_renderer_frame_begin(
				state.context, state.display.physical_width, state.display.physical_height)) {
			// Shadow pass
			vulkan_renderer_pass_begin(state.context, RENDERER_DEFAULT_PASS_SHADOW);
			{
				mat4 light_projection;
				float scene_radius = 30.0f;
				glm_ortho(-scene_radius, scene_radius, scene_radius, -scene_radius, 1.0f,
					scene_radius * 2.0f, light_projection);
				...
				vulkan_renderer_resource_global_write(
					state.context, RENDERER_GLOBAL_RESOURCE_SHADOW, 0, sizeof(mat4), &light_matrix);
				vulkan_renderer_resource_global_bind(
					state.context, RENDERER_GLOBAL_RESOURCE_SHADOW);
				vulkan_renderer_shader_bind(
					state.context, shadow_shader_index, RENDERER_DEFAULT_SHADER_VARIANT_STANDARD);
			...
			vulkan_renderer_texture_prepare_sample(
			...
			vulkan_renderer_pass_begin(state.context, RENDERER_DEFAULT_PASS_MAIN);
			{
				...
				for (uint32_t light_index = 0; light_index < countof(lights); ++light_index) {
					if (lights[light_index].type == LIGHT_TYPE_DIRECTIONAL)
						...
						memcpy(frame_data.lights + point_light_count++, lights + light_index,
							sizeof(Light));
				}
				glm_vec3_dup(state.editor_camera.position, frame_data.camera_position);
				frame_data.light_count = point_light_count;

				vulkan_renderer_resource_global_write(state.context, RENDERER_GLOBAL_RESOURCE_MAIN,
					0, sizeof(FrameData), &frame_data);
				...
				for (uint32_t index = 0; index < countof(lights); ++index) {
					...
					vulkan_renderer_resource_group_write(state.context, light_materials[index], 0,
			...
			vulkan_renderer_pass_end(state.context);
			vulkan_renderer_texture_prepare_sample(
				state.context, RENDERER_DEFAULT_TARGET_MAIN_COLOR_MAP);

			// Postfx pass
			vulkan_renderer_pass_begin(state.context, RENDERER_DEFAULT_PASS_POSTFX);
			{
				vulkan_renderer_shader_bind(
					state.context, postfx_shader_index, RENDERER_DEFAULT_SHADER_VARIANT_STANDARD);
				vulkan_renderer_resource_global_bind(
...
void editor_update(float dt) {
	...
	glm_vec3_muladds(camera_right,
		(input_key_down(SV_KEY_D) - input_key_down(SV_KEY_A)) * CAMERA_MOVE_SPEED * dt, move);
	glm_vec3_muladds(camera_down,
		(input_key_down(SV_KEY_SPACE) - input_key_down(SV_KEY_C)) * CAMERA_MOVE_SPEED * dt, move);
	glm_vec3_muladds(target_position,
		(input_key_down(SV_KEY_S) - input_key_down(SV_KEY_W)) * CAMERA_MOVE_SPEED * dt, move);
...
bool resize_event(Event *event) {
	...
	vulkan_renderer_texture_resize(
		state.context, RENDERER_DEFAULT_TARGET_SHADOW_DEPTH_MAP, state.width, state.height);
	vulkan_renderer_texture_resize(
		state.context, RENDERER_DEFAULT_TARGET_MAIN_COLOR_MAP, state.width, state.height);
	vulkan_renderer_texture_resize(
		state.context, RENDERER_DEFAULT_TARGET_MAIN_DEPTH_MAP, state.width, state.height);

	vulkan_renderer_resource_global_set_texture_sampler(state.context,
		RENDERER_GLOBAL_RESOURCE_POSTFX, 0, state.final_texture, RENDERER_DEFAULT_SAMPLER_NEAREST);
...
UUID create_plane_mesh(Arena *arena, uint32_t subdivide_width, uint32_t subdivide_depth,
	...
	String name =
		string_format(arena, "plane_%ux%u_d", subdivide_width, subdivide_depth, orientation);
	...
	for (uint32_t column = 0; column < columns; ++column) {
		for (uint32_t row = 0; row < rows; ++row) {
			...
			// Vertex v00 = { .position = { 0.0f }, .normal = { 0.0f, 1.0f, 0.0f }, .uv = { 0.0f,
			// 0.0f }, .tangent = { 0.0f, 0.0f, 1.0f, 1.0f } }; Vertex v10 = { .position = { 0.0f },
			// .normal = { 0.0f, 1.0f, 0.0f }, .uv = { 1.0f, 0.0f }, .tangent = { 0.0f,
			// 0.0f, 1.0f, 1.0f } }; Vertex v01 = { .position = { 0.0f }, .normal = { 0.0f, 1.0f,
			// 0.0f }, .uv = { 0.0f, 1.0f }, .tangent = { 0.0f, 0.0f, 1.0f, 1.0f } }; Vertex v11 = {
			// .position = { 0.0f }, .normal = { 0.0f, 1.0f, 0.0f }, .uv = { 1.0f, 1.0f }, .tangent
			// = { 0.0f, 0.0f, 1.0f, 1.0f } };

			Vertex orientation_y_vertex00 = { .position = { rowf, 0, columnf },
				.normal = { 0.0f, 1.0f, 0.0f },
				.uv = { 0.0f, 0.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_y_vertex10 = { .position = { rowf + row_unit, 0, columnf },
				.normal = { 0.0f, 1.0f, 0.0f },
				.uv = { 1.0f, 0.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_y_vertex01 = { .position = { rowf, 0, columnf + column_unit },
				.normal = { 0.0f, 1.0f, 0.0f },
				.uv = { 0.0f, 1.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_y_vertex11 = { .position = { rowf + row_unit, 0,
												columnf + column_unit },
				.normal = { 0.0f, 1.0f, 0.0f },
				.uv = { 1.0f, 1.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };

			Vertex orientation_x_vertex00 = { .position = { 0, columnf + column_unit, rowf },
				.normal = { 1.0f, 0.0f, 0.0f },
				.uv = { 0.0f, 0.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_x_vertex10 = { .position = { 0, columnf + column_unit,
												rowf + row_unit },
				.normal = { 1.0f, 0.0f, 0.0f },
				.uv = { 0.0f, 1.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_x_vertex01 = { .position = { 0, columnf, rowf },
				.normal = { 1.0f, 0.0f, 0.0f },
				.uv = { 1.0f, 0.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_x_vertex11 = { .position = { 0, columnf, rowf + row_unit },
				.normal = { 1.0f, 0.0f, 0.0f },
				.uv = { 1.0f, 1.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };

			Vertex orientation_z_vertex00 = { .position = { rowf, columnf + column_unit, 0 },
				.normal = { 0.0f, 0.0f, 1.0f },
				.uv = { 0.0f, 0.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_z_vertex10 = { .position = { rowf + row_unit, columnf + column_unit,
												0 },
				.normal = { 0.0f, 0.0f, 1.0f },
				.uv = { 1.0f, 0.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_z_vertex01 = { .position = { rowf, columnf, 0 },
				.normal = { 0.0f, 0.0f, 1.0f },
				.uv = { 0.0f, 1.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
			Vertex orientation_z_vertex11 = { .position = { rowf + row_unit, columnf, 0 },
				.normal = { 0.0f, 0.0f, 1.0f },
				.uv = { 1.0f, 1.0f },
				.tangent = { 0.0f, 0.0f, 1.0f, 1.0f } };
...
bool game_load(GameContext *context) {
	...
	String temp_path = string_format(scratch.arena, "./%s.so", "game");

...
int32_t main(int32_t argc, const char *argv[]) {
	...
	OS_Library game = os_library_load(s("game.so"));
	...
	while (is_open) {
			...
			switch (event.type) {
				...
				case OS_EVENT_TYPE_KEY_RELEASE:
					if (event.as.key.key_code == KEY_CODE_ESCAPE)
						is_open = false;
					if (event.as.key.key_code == KEY_CODE_T) {
						if (hidden == false) {
							os_surface_hide(popup);
							hidden = true;
						} else {
							os_surface_show(popup);
							hidden = false;
						}
					}
		...
		if (timestamp != os_file_last_modified(s("game.so"))) {
			...
			game = os_library_load(s("game.so"));
			os_library_symbol(game, s("game_hook"), &hookup);
			timestamp = os_file_last_modified(s("game.so"));
		}
	}

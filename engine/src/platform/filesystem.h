...
ENGINE_API Span filesystem_read(struct arena *arena, String path);
...
#define file_write_struct(file, T, ...)                  \
	do {                                                 \
		...
		filesystem_write((file), sizeof(T), 1, &_value); \
	} while (0)
#define file_write_count(file, count, data) filesystem_write(file, sizeof(*data), count, data)

...
INLINE void draw2d_circle(Arena *arena, float2 position, float radius, Color color) { draw2d_quad(arena, rect(position.x - radius, position.y - radius, radius * 2.0, radius * 2.0), (DRAW_QuadStyle){ .fill_color = color, .origin = splat2(radius), .radii = splat4(radius) }); }
...
void draw2d_arrow(Arena *arena, float2 origin, float2 delta, float thickness, float head_lengh, Color color);
/* void draw2d_arrow_between(Arena *arena, float2 a, float b, float thickness, float head_length, Color color);  */

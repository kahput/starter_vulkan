...
static inline Arena arena_wrap_buffer(Bytes buffer) { return (Arena){ .base = buffer.memory, .capacity = buffer.size }; }
...
ENGINE_API ArenaTrieNode *arena_trienode_ensure(Arena *arena, ArenaTrieNode **root, Bytes key, const char *debug_type_name);
ENGINE_API void *arena_triestruct_ensure(Arena *arena, ArenaTrieHeader **root, size_t key_offset, size_t value_offset, Bytes key, size_t map_size, size_t map_align);
ENGINE_API void *arena_trie_ensure(Arena *arena, ArenaTrieNode **root, Bytes key, size_t size, size_t align, const char *debug_type_name);

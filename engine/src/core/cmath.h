...
INLINE float2 load2(float v[2]) { return (float2){ v[0], v[1] }; }
...
INLINE float len2_sq(float2 v) { return dot2(v, v); }
...
INLINE float3 load3(float v[3]) { return (float3){ v[0], v[1], v[2] }; }
...
INLINE float4 load4(float v[4]) { return (float4){ .x = v[0], .y = v[1], .z = v[2], .w = v[3] }; }

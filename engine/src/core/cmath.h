...
INLINE bool equal2(float2 a, float2 b) { return equalf(a.x, b.x) && equalf(a.y, b.y); }
...
INLINE float min3(float3 v) { return minf(v.x, minf(v.y, v.z)); }
INLINE float max3(float3 v) { return maxf(v.x, maxf(v.y, v.z)); }
...
INLINE bool equal4(float4 a, float4 b) { return equalf(a.x, b.x) && equalf(a.y, b.y) && equalf(a.z, b.z) && equalf(a.w, b.w); }
...
float4x4 rotate4x4(float4x4 matrix, float radians_angle, float3 axis);
float4x4 scale4x4(float4x4 matrix, float3 scale);

float4x4 make4x4_from_translation(float3 translation);
...
float4x4 compose4x4_from_euler(float3 position, float3 radians_angle, float3 scale);

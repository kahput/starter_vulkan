...
static inline bool gfx_buffer_valid(GFX_Device *device, GFX_Buffer *buffer) {
	return gfx_device_valid(device) && buffer && buffer > device->buffer_pool && buffer < device->buffer_pool + MAX_BUFFERS && buffer->next == device->buffer_pool;
}

static inline bool gfx_image_valid(GFX_Device *device, GFX_Image *image) {
	return gfx_device_valid(device) && image && image > device->image_pool && image < device->image_pool + MAX_IMAGES && image->next == device->image_pool;
}

static inline bool gfx_sampler_valid(GFX_Device *device, GFX_Sampler *sampler) {
	return gfx_device_valid(device) && sampler && sampler > device->sampler_pool && sampler < device->sampler_pool + MAX_SAMPLERS && sampler->next == device->sampler_pool;
}

static inline bool gfx_swapchain_valid(GFX_Device *device, GFX_Swapchain *swapchain) {
	return gfx_device_valid(device) && swapchain && swapchain > device->swapchain_pool && swapchain < device->swapchain_pool + MAX_SWAPCHAINS && swapchain->next == device->swapchain_pool;
}

static inline bool gfx_pipeline_valid(GFX_Device *device, GFX_Pipeline *shader) {
	return gfx_device_valid(device) && shader && shader->next == device->shader_pool; // ;&& shader && shader > device->shader_pool && shader < device->shader_pool + MAX_SHADERS && shader->next == device->shader_pool;
}
...
GFX_Pipeline compute_pipeline_make(GFX_Device *device, String8 bytecode);
GFX_Pipeline graphics_pipeline_make(GFX_Device *device, String8 vs_bytecode, String8 fs_bytecode, PipelineOptions options);
...
bool gfx_bind(GFX_Device *device, GFX_Pipeline *pipeline, uint32_t set, Uniform *uniforms, uint32_t uniform_count);
